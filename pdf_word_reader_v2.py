...
class PDFWordReaderApp:
    def __init__(self, root):
        ...
        self.word_map_canvas.bind("<Button-1>", self.on_word_map_click)
        self.word_map_canvas.bind("<Configure>", self.on_word_map_resize)
    ...
    def build_ui(self):
        ...
        self.sidebar = tk.Frame(self.content_frame, width=360)
        ...
        self.page_listbox = tk.Listbox(self.sidebar, exportselection=False, height=10)
        ...
        self.preview_frame = tk.Frame(self.sidebar, bd=1, relief="sunken", height=420)
        self.preview_frame.pack(fill="both", expand=True, padx=5, pady=(0, 8))
        self.preview_frame.pack_propagate(False)

        self.preview_label = tk.Label(self.preview_frame, text="Preview unavailable")
        self.preview_label.pack(fill="both", expand=True)

        self.word_map_title = tk.Label(self.sidebar, text="Word Map")
        self.word_map_title.pack(anchor="w", padx=5, pady=(4, 5))

        self.word_map_hint = tk.Label(
            self.sidebar,
            text="Click a position in the map to jump to that word on the previewed page.",
            justify="left",
            wraplength=320,
            anchor="w",
        )
        self.word_map_hint.pack(anchor="w", padx=5, pady=(0, 6), fill="x")

        self.word_map_canvas = tk.Canvas(self.sidebar, height=92, bd=1, relief="sunken", highlightthickness=0)
        self.word_map_canvas.pack(fill="x", padx=5, pady=(0, 6))

        self.word_map_status_var = tk.StringVar(value="Word map unavailable")
        self.word_map_status_label = tk.Label(self.sidebar, textvariable=self.word_map_status_var, anchor="w")
        self.word_map_status_label.pack(fill="x", padx=5, pady=(0, 2))
    ...
    def restore_last_position(self, show_status=False):
        ...
        if self.current_index > 0:
            ...
            self.render_word_map(start_page)
    ...
    def load_pdf(self, silent=False):
        ...
        try:
            ...
            for i, page in enumerate(reader.pages, start=1):
                ...
                page_entries = [
                    {"word": word, "page": i, "page_word_index": idx}
            ...
            self.word_map_status_var.set("Word map unavailable")
    ...
    def get_current_display_token(self):
        idx = self.get_current_display_index()
        if idx < 0 or idx >= len(self.play_tokens):
            return None
        return self.play_tokens[idx]
    ...
    # -----------------------------
    # Progress, WPM, page highlight, preview, word map
    # -----------------------------
    ...
    def highlight_page(self, page_number):
        if self.page_listbox.size() == 0 or page_number is None:
            return
        idx = page_number - 1
    ...
    def get_page_tokens(self, page_number):
        if page_number is None or page_number < 1 or page_number > len(self.page_tokens):
            return []
        return self.page_tokens[page_number - 1]

    def render_word_map(self, page_number):
        self.word_map_canvas.delete("all")

        canvas_width = max(self.word_map_canvas.winfo_width(), 80)
        canvas_height = max(self.word_map_canvas.winfo_height(), 70)

        if page_number is None or self.total_pages <= 0:
            self.word_map_status_var.set("Word map unavailable")
            self.word_map_canvas.create_text(
                canvas_width / 2,
                canvas_height / 2,
                text="No page selected",
                anchor="center",
                fill=self.get_theme_color("muted_fg"),
                font=("Arial", 10),
            )
            return

        tokens = self.get_page_tokens(page_number)
        total_words = len(tokens)

        if total_words == 0:
            self.word_map_status_var.set(f"Page {page_number}: 0 readable words")
            self.word_map_canvas.create_rectangle(8, 8, canvas_width - 8, canvas_height - 8, outline=self.get_theme_color("border"))
            self.word_map_canvas.create_text(
                canvas_width / 2,
                canvas_height / 2,
                text=f"Page {page_number} has no readable words",
                anchor="center",
                fill=self.get_theme_color("muted_fg"),
                font=("Arial", 10),
            )
        ...
        top_pad = 20
        bottom_pad = 18
        plot_w = max(20, canvas_width - left_pad - right_pad)
        plot_h = max(12, canvas_height - top_pad - bottom_pad)

        self.word_map_canvas.create_rectangle(
            left_pad,
            top_pad,
            left_pad + plot_w,
            top_pad + plot_h,
            outline=self.get_theme_color("border"),
            fill=self.get_theme_color("map_bg"),
        )

        bins = min(max(24, plot_w), total_words, 240)
        counts = [0 for _ in range(bins)]
        for idx in range(total_words):
            bin_idx = min(bins - 1, int((idx / max(1, total_words)) * bins))
            counts[bin_idx] += 1
        ...
        for i, count in enumerate(counts):
            ...
            self.word_map_canvas.create_rectangle(
                ...
                y1,
                outline="",
                fill=self.get_theme_color("map_fill"),
            )

        current_token = self.get_current_display_token()
        current_word_index = None
        if current_token and current_token.get("page") == page_number:
            current_word_index = current_token.get("page_word_index")

        if current_word_index is not None and total_words > 0:
            marker_x = left_pad + (current_word_index / max(1, total_words - 1)) * plot_w
            self.word_map_canvas.create_line(
                marker_x,
                top_pad - 2,
                marker_x,
                top_pad + plot_h + 2,
                fill=self.get_theme_color("marker"),
                width=2,
            )
            self.word_map_canvas.create_oval(
                marker_x - 4,
                top_pad - 8,
                marker_x + 4,
                top_pad,
                fill=self.get_theme_color("marker"),
                outline=self.get_theme_color("marker"),
            )
        ...
        if current_word_index is not None:
            self.word_map_status_var.set(
                f"Page {page_number}: word {current_word_index + 1} of {total_words}. Click map to jump."
            )
        else:
            self.word_map_status_var.set(f"Page {page_number}: {total_words} words. Click map to jump.")

    def find_play_index_for_page_word(self, page_number, page_word_index):
        if not self.play_tokens:
            return None
        for idx, token in enumerate(self.play_tokens):
            if token.get("page") == page_number and token.get("page_word_index") == page_word_index:
                return idx
        return None
    ...
    def preview_selected_page(self, page_number):
        ...
        self.render_word_map(page_number)
        self.page_status_var.set(f"Current Page: {page_number}")

    def jump_to_page(self, page_number, autoplay=None):
        return self.jump_to_page_word(page_number, 0, autoplay=autoplay)

    def jump_to_page_word(self, page_number, page_word_index=0, autoplay=None):
        ...
        page_entries = self.get_page_tokens(page_number)
        if not page_entries:
            ...
            self.render_page_preview(page_number)
            self.render_word_map(page_number)
            self.progress_var.set(f"Word 0 / {len(self.play_tokens)}" if self.play_tokens else "Word 0 / 0")
            self.set_progress(0, len(self.play_tokens))
            self.status_var.set(f"Page {page_number} has no readable words.")
            self.save_preferences()
            return False

        if page_word_index < 0:
            page_word_index = 0
        if page_word_index >= len(page_entries):
            page_word_index = len(page_entries) - 1
        ...
        target_index = self.find_play_index_for_page_word(page_number, page_word_index)
        ...
        self.session_start_time = None
        self.session_displayed_words = 0
        self.display_token_at(target_index, count_for_wpm=False, persist=False)
        self.current_index = target_index + 1
        self.update_wpm_labels()
        self.status_var.set(
            f"Jumped to page {page_number}, word {page_word_index + 1} of {len(page_entries)}."
        )
        self.save_preferences()
    ...
    def on_word_map_click(self, event=None):
        if self.total_pages <= 0:
            return
        page_number = self.preview_page_number or self.safe_get_start_page()
        tokens = self.get_page_tokens(page_number)
        ...
        right_pad = 10
        plot_w = max(20, canvas_width - left_pad - right_pad)
        click_x = getattr(event, "x", left_pad)
        relative = (click_x - left_pad) / plot_w
        if relative < 0:
            relative = 0.0
        if relative > 1:
            relative = 1.0
        page_word_index = int(round(relative * max(0, len(tokens) - 1)))
        self.jump_to_page_word(page_number, page_word_index, autoplay=None)

    def on_word_map_resize(self, event=None):
        if self.preview_page_number is not None:
            self.render_word_map(self.preview_page_number)
        else:
            self.render_word_map(None)
    ...
    def apply_theme(self):
        bg_main = self.get_theme_color("bg_main")
        bg_panel = self.get_theme_color("bg_panel")
        bg_display = self.get_theme_color("bg_display")
        fg_text = self.get_theme_color("fg")
        entry_bg = self.get_theme_color("entry_bg")
        entry_fg = self.get_theme_color("entry_fg")
        word_bg = self.get_theme_color("word_bg")
        word_fg = self.get_theme_color("word_fg")
        list_bg = self.get_theme_color("list_bg")
        list_fg = self.get_theme_color("list_fg")
        select_bg = self.get_theme_color("select_bg")
        select_fg = self.get_theme_color("select_fg")
        map_bg = self.get_theme_color("bg_display")
        ...
        self.word_map_canvas.configure(bg=map_bg)
    ...
    def on_page_list_select(self, event=None):
        try:
            ...
            self.status_var.set(f"Previewing page {page_num}. Double-click preview or click word map to jump reading there.")
        except Exception:
            pass
